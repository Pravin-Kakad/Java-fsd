...
//		}
	...
	public void addCrossing(RailwayCrossing crossing) {
        try {
            String query = "INSERT INTO railway_crossing (name, address, landmark, trainSchedule, personInCharge, status) VALUES (?, ?, ?, ?, ?, ?)";
            PreparedStatement statement = connection.prepareStatement(query);
            statement.setString(1, crossing.getName());
            statement.setString(2, crossing.getAddress());
            statement.setString(3, crossing.getLandmark());
            ...
            if (rowsInserted > 0) {
                System.out.println("A new railway crossing was inserted successfully!");
            }
